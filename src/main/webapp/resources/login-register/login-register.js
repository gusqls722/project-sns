...
function SignIn() {
	if (SandR == 0) { //로그인을 눌렀을 때
		...
		alert(email.value);
		var emailconf = {
			url : '/uis/emailcheck?email=' + decodeURIComponent(email.value),
			method : 'GET',
			success : function(res) {
				res = JSON.parse(res);
				alert(res.msg);
...
function validate() { // 모든 값을 다 입력했으면 버튼 활성화
	...
	if (SandR == 0) { //로그인일때
		...
		if (id.value != "" && pwd.value != "" && cpwd.value != ""
				&& nameObj.value != "" && email.value != ""
				&& birth.value != "" && gender.value != "" && pwdChecking == 1
				&& pwdLenCheck == 1 && idcheckcount == 0) {
			btn.disabled = false;
